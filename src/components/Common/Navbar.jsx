...
const Navbar = () => {
  ...
  useEffect(() => {
    const getCurrentUserDetails = async () => {
      setLoading(true);
      if (token) {
        await getUserDetails(token, dispatch, navigate);
      }
      setLoading(false);
    }
                                    ...
                                    <div className='flex flex-col capitalize' >
                                      {
                                        catalogs.map((catalog, index) => (
                                          <Link to={`/catalog/${catalog.name.split(" ").join("-").toLowerCase()}`} key={index} >
                      ...
                      catalogs.length ?
                        (
                          <div className='flex flex-col capitalize' >
                            {
                              catalogs.map((catalog, index) => (
                                <Link to={`/catalog/${catalog.name.split(' ').join('-').toLowerCase()}`} key={index} onClick={() => setIsMenuModalOpen(false)} >
